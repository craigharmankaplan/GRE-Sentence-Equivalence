...
export default async (req) => {
    ...
    const { step, sentence, rubric, studentInput, model,
            correctPair, selections, clueAnalysis, prediction,
            questions } = body || {};

    if (![1, 2, 4, 5].includes(step)) return json({ error: 'step must be 1, 2, 4, or 5' }, 400);
    ...
    if (step === 5) {
        if (!Array.isArray(questions) || questions.length === 0) return json({ error: 'questions must be a non-empty array' }, 400);
    }
...
function buildPromptAndSchema(step, payload) {
    ...
    // step === 5: end-of-session recap across all questions
    const { questions: qs } = payload;
    const sessionBlob = qs.map(function(q, i) {
        return {
            q: i + 1,
            sentence: q.sentence,
            correctPair: q.correctPair,
            clueAnalysis: q.clueAnalysis,
            prediction: q.prediction,
            selections: q.selections,
            correctFirstTry: q.correctFirstTry,
            aiStep1: q.aiStep1 || null,
            aiStep2: q.aiStep2 || null
        };
    });

    const prompt =
        'You are a GRE verbal coach writing a brief end-of-session recap after a student completed a practice set of Sentence Equivalence questions. The Kaplan Method has four steps: (1) find clues, (2) predict a word, (3) match to answer choices, (4) confirm by reading back.\n\n' +
        `SESSION DATA (${qs.length} questions): ${JSON.stringify(sessionBlob)}\n\n` +
        'Write a warm, encouraging recap (40-70 words). Follow these rules strictly:\n\n' +
        '1. OPEN with a generic positive statement like "Nice work in this session." (vary the wording but keep it warm).\n' +
        '2. FOCUS on process and method (the four steps: clues, predict, match, confirm), NOT on how many they got right or wrong.\n' +
        '3. Do NOT comment on overall performance UNLESS one of these is true:\n' +
        ...
        'Output ONLY a JSON object. No preamble, no markdown fences.\n' +
        '- summary (string): the recap text, addressed to "you".';
